...
// 这段代码是实现了椭圆曲线上元素的平方根计算。具体来说，对于有限域Fq中的一个元素el，该函数返回一个Fq中的元素x，使得x^2 = el。
// 该函数采用了Tonelli-Shanks算法，该算法是一种计算Fq上平方根的经典算法，它利用了Fq上的二次剩余的性质。该算法分为两个阶段，第一个阶段利用欧拉准则（Euler's Criterion）检查元素是否是二次剩余，如果不是则无法求出平方根；第二个阶段是计算平方根的具体过程。
// 该函数中的变量b、w、x、z分别对应算法中的变量b、w、x、z。其中，变量b表示当前的平方剩余，变量w、z表示Tonelli-Shanks算法中的中间值，变量x表示当前的解。
...
/*
bn128_G1::mixed_add(const bn128_G1 &other)函数的功能是

对一个已知在仿射坐标系下的椭圆曲线上的点(this)和另一个在射影坐标系下的椭圆曲线上的点other进行加法运算，
结果为一个在射影坐标系下的点。其中，已知点this可以写成(X1, Y1)，另一个点other可以写成(X2 : Y2 : Z2)。
